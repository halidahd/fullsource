...
$socialProfiles = array('Facebook', 'Twitter', 'Google Plus', 'Instagram', 'RSS', 'Email', 'Youtube', 'Vimeo', 'Flickr', 'Foursquare', 'Pinterest', 'StumbleUpon');
class wpzoom_widget_socialize extends WP_Widget {

/* Widget setup. */
function wpzoom_widget_socialize() {
	/* Widget settings. */
	$widget_ops = array( 'classname' => 'wpzoom-social', 'description' => __('Custom WPZOOM Widget that displays links to social sharing websites.', 'wpzoom') );
		...
		foreach ($socialProfiles as $item)
		{
			...
			?>
 	 		<p>
				...
				<input style="margin-left:31px;" type="text" id="<?php echo $this->get_field_id( $id ); ?>" name="<?php echo $this->get_field_name( $id ); ?>" value="<?php echo $instance[$id]; ?>" size="23"/> 
