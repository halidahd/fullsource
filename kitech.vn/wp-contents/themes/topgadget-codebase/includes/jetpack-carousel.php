...
class No_Jetpack_Carousel {
	...
	function init() {
		...
		if ( is_admin() ) {
			...
			add_filter( 'post_gallery', array(
					$this,
					'enqueue_assets'
				), 1000, 2 ); // load later than other callbacks hooked it
		...
		if ( $this->in_jetpack && method_exists( 'Jetpack', 'module_configuration_load' ) ) {
			...
			Jetpack::module_configuration_load( dirname( dirname( __FILE__ ) ) . '/carousel.php', array(
					$this,
	...
	function enqueue_assets( $output ) {
		...
		if ( $this->first_run ) {
			if ( ! has_action( 'wp_enqueue_scripts', 'register_spin_scripts' ) ) {
				...
				wp_enqueue_script( 'spin', get_bloginfo( 'template_directory' ) . '/js/spin.js', false, '1.2.4' );
				wp_enqueue_script( 'jquery.spin', get_bloginfo( 'template_directory' ) . '/js/jquery.spin.js', array(
						'jquery',
						'spin'
					) );
			}

			wp_enqueue_script( 'jetpack-carousel', get_bloginfo( 'template_directory' ) . '/js/jetpack-carousel.js', $this->asset_version( '20120629' ), true );
			...
			wp_enqueue_style( 'jetpack-carousel', get_bloginfo( 'template_directory' ) . '/includes/jetpack-carousel.css', array(), $this->asset_version( '20120629' ) );
	...
	function register_settings() {
		add_settings_section( 'carousel_section', __( 'Image Gallery Carousel', 'carousel' ), array(
				$this,
				'carousel_section_callback'
			), 'media' );

		if ( ! $this->in_jetpack ) {
			add_settings_field( 'carousel_enable_it', __( 'Enable carousel', 'carousel' ), array(
					$this,
		...
		add_settings_field( 'carousel_background_color', __( 'Background color', 'carousel' ), array(
				$this,
		...
		add_settings_field( 'carousel_display_exif', __( 'Metadata', 'carousel' ), array(
				$this,
	...
	function carousel_background_color_callback() {
		$this->settings_select( 'carousel_background_color', array(
				'black' => __( 'Black', 'carousel' ),
				'white' => __( 'White', 'carousel', 'carousel' )
			) );
	}
