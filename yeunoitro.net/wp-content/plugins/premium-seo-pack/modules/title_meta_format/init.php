 			...
 			if (in_array($type, array('post', 'page'))) {
				...
				$__wpquery = ( !empty($__postType) && $__postType == 'post' ? $this->post : $wp_query->get_queried_object() );
 				...
 				if ( isset($__postClean['id']) && !is_null($__postClean['id']) && $__postClean['id']>0 ) {
					...
					//halh, add focus keyword if keywords is null
					if(!$__postClean['keywords'] && isset($__postClean['focus_keywords'])) $__postClean['keywords'] = $__postClean['focus_keywords'];

					//halh edit post description
					$__postClean['description'] = isset($__tmpKeywords['description']) ? $__tmpKeywords['description'] : $__postClean['description'];
 				}
 			}
