...
class topgadget_Theme_Updater {
	...
	public function updater_data() {
		...
		/* get updater config */
		if ( ! is_array( $theme_support[0] ) ) {
			$user_config = false;
		} else {
			$user_config = $theme_support[0];
		}
		...
		if ( ! empty( $theme_uri ) ) {
			$theme_uri = esc_url_raw( $theme_uri );
		}
	...
	public function transient_update_themes( $checked_data ) {
		...
		$remote_url     = add_query_arg( array(
				'theme_repo' => $updater_data['repo_slug'],
				'ahtr_check' => $updater_data['version']
	...
	public function dashboard_widget_control_callback() {
		...
		<?php if ( true === $updater_data['role'] ) { // members only update ?>
			...
			<p>
				<input id="<?php echo $widget_id; ?>-username" name="<?php echo $widget_id; ?>[username]" type="text"
					   value="<?php echo $username_option; ?>"/>
			</p>
			<p>
