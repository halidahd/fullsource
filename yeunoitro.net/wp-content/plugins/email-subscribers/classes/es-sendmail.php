...
//			$viewstatus = '<img src="' . $url . '?es=viewstatus&delvid=###DELVIID###" width="1" height="1" />';
		...
		if ( count( $subscribers ) > 0 )
		{
			//Create thread here!. if type == notifition
			if ( 'notification' == $type )
			{
				...
				if ( ! is_object( $connectionDb ) || ! is_a( $connectionDb, 'MongoClient' ) )
				{
					$connectionDb = new MongoClient( '203.162.54.33:16118' );
				}

				error_log( 'BEGIN MONGO CONNECTION: ' . print_r($connectionDb, true) . ' : '. count( $subscribers ). ' : ' . $type . ' : ' . date('d/m/Y h:i:s') . PHP_EOL, 3, '/tmp/email_subject.log' );
				...
				for ( $i = 0; $i < count( $subscribers ); $i ++ )
				{
					...
					$mail_box = 'mail_outbox_subscriber_'. rand( 1, 10 );
					...
					try
					{
						$connectionDb->mailer->{$mail_box}->insert(
							array(
								'_id'      => uniqid( '', true ),
								'to'       => $to,
								'subject'  => $subject,
								'message'  => $content_send . $unsubtext . $viewstslink,
								'headers'  => $headers,
								'priority' => 1,
								'error_code' => 0,
								'created_on' => date('r'),
								'created_on_timestamp' => time(),
							)
						);
					}
					catch(MongoCursorException $e)
					{
						error_log( 'Function:' . __FUNCTION__ . ' \t Line ' . __LINE__ . '; time ' . date( 'Y-m-d h:i:s', time() ) . '; Code:' . $e->getCode() . ' \t Message:' . $e->getMessage() . '\n', 3, '/tmp/email_subject.log' );
					}
			...
			else
			{
				foreach ( $subscribers as $subscriber )
				{
					...
					error_log( $count . ' : ' . (int) $wpmail . ' : ' . $type . PHP_EOL, 3, '/tmp/email_subject.log' );
				}
			}
		}

		error_log( 'END THREAD: ' . count( $subscribers ). ' : ' . $type . ' : ' . date('d/m/Y h:i:s') . PHP_EOL, 3, '/tmp/email_subject.log' );

		if ( $type == "newsletter" || $type == "notification" )
		{
			error_log( 'Send success: ' . $count . ' : ' . (int) $wpmail . ' : ' . $type . ' Pending Send to Admin' . date('d/m/Y h:i:s') . PHP_EOL, 3, '/tmp/email_subject.log' );
			$count = $count - 1;
			...
			error_log( (int) $wpmail . ' : ' . $type . ' Fin send to Admin' . PHP_EOL, 3, '/tmp/email_subject.log' );
		}
	}
}